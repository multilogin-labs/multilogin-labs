# Cookbook 44: Selenium explicit waits pattern
...
Selenium explicit waits pattern — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- selenium wait explicit
# or
npx mlx-labs search selenium wait explicit
```
...
// Implement: selenium explicit waits pattern
```
...
**Keywords:** multilogin api cookbook · selenium wait explicit
