# Cookbook 45: Record + replay session events
...
Record + replay session events — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- session replay record
# or
npx mlx-labs search session replay record
```
...
// Implement: record + replay session events
```
...
**Keywords:** multilogin api cookbook · session replay record
