# Cookbook 46: Run anti-bot tests on a profile
...
Run anti-bot tests on a profile — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- antibot fingerprint test
# or
npx mlx-labs search antibot fingerprint test
```
...
// Implement: run anti-bot tests on a profile
```
...
**Keywords:** multilogin api cookbook · antibot fingerprint test
