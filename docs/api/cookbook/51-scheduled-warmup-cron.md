# Cookbook 51: Scheduled warmup with cron
...
Scheduled warmup with cron — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- warmup cron schedule
# or
npx mlx-labs search warmup cron schedule
```
...
// Implement: scheduled warmup with cron
```
...
**Keywords:** multilogin api cookbook · warmup cron schedule
