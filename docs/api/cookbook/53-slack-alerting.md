# Cookbook 53: Slack alert on profile errors
...
Slack alert on profile errors — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- slack alert webhook
# or
npx mlx-labs search slack alert webhook
```
...
// Implement: slack alert on profile errors
```
...
**Keywords:** multilogin api cookbook · slack alert webhook
