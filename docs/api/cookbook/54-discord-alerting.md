# Cookbook 54: Discord webhook for status pings
...
Discord webhook for status pings — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- discord alert webhook
# or
npx mlx-labs search discord alert webhook
```
...
// Implement: discord webhook for status pings
```
...
**Keywords:** multilogin api cookbook · discord alert webhook
