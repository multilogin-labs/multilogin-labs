# Cookbook 56: Export profile status to CSV
...
Export profile status to CSV — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- csv export reporting
# or
npx mlx-labs search csv export reporting
```
...
// Implement: export profile status to csv
```
...
**Keywords:** multilogin api cookbook · csv export reporting
