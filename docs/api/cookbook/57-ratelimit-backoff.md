# Cookbook 57: Exponential backoff for rate limits
...
Exponential backoff for rate limits — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- ratelimit backoff retry
# or
npx mlx-labs search ratelimit backoff retry
```
...
**Keywords:** multilogin api cookbook · ratelimit backoff retry
