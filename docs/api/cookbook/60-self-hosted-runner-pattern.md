# Cookbook 60: Self-hosted CI runner pattern
...
Self-hosted CI runner pattern — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- self hosted runner ci
# or
npx mlx-labs search self hosted runner ci
```
...
// Implement: self-hosted ci runner pattern
```
...
**Keywords:** multilogin api cookbook · self hosted runner ci
