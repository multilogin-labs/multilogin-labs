# Cookbook 61: Run warmup from GitHub Actions cron
...
Run warmup from GitHub Actions cron — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- github actions cron warmup
# or
npx mlx-labs search github actions cron warmup
```
...
# Run warmup from GitHub Actions cron — implement the flow with the endpoints printed above
```
...
// run warmup from github actions cron — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · github actions cron warmup
