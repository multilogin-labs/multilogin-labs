# Cookbook 68: Send timings to Datadog
...
Send timings to Datadog — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- datadog statsd metrics
# or
npx mlx-labs search datadog statsd metrics
```
...
# Send timings to Datadog — implement the flow with the endpoints printed above
```
...
// send timings to datadog — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · datadog statsd metrics
