# Cookbook 69: Capture errors to Sentry
...
Capture errors to Sentry — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- sentry error tracking
# or
npx mlx-labs search sentry error tracking
```
...
# Capture errors to Sentry — implement the flow with the endpoints printed above
```
...
// capture errors to sentry — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · sentry error tracking
