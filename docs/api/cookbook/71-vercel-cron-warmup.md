# Cookbook 71: Vercel cron triggers warmup
...
Vercel cron triggers warmup — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- vercel cron serverless
# or
npx mlx-labs search vercel cron serverless
```
...
# Vercel cron triggers warmup — implement the flow with the endpoints printed above
```
...
// vercel cron triggers warmup — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · vercel cron serverless
