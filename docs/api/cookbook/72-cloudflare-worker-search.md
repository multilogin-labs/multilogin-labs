# Cookbook 72: Cloudflare Worker proxies search
...
Cloudflare Worker proxies search — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- cloudflare worker search
# or
npx mlx-labs search cloudflare worker search
```
...
# Cloudflare Worker proxies search — implement the flow with the endpoints printed above
```
...
// cloudflare worker proxies search — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · cloudflare worker search
