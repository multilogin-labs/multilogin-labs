# Cookbook 76: Sync profile metadata to Supabase
...
Sync profile metadata to Supabase — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- supabase sync postgres
# or
npx mlx-labs search supabase sync postgres
```
...
# Sync profile metadata to Supabase — implement the flow with the endpoints printed above
```
...
// sync profile metadata to supabase — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · supabase sync postgres
