# Cookbook 77: Sync profile metadata to Airtable
...
Sync profile metadata to Airtable — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- airtable sync metadata
# or
npx mlx-labs search airtable sync metadata
```
...
# Sync profile metadata to Airtable — implement the flow with the endpoints printed above
```
...
// sync profile metadata to airtable — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · airtable sync metadata
