# Cookbook 80: Plug rotating residential proxy provider
...
Plug rotating residential proxy provider — production-ready pattern using Multilogin X API.
...
```bash
npm run api:search -- rotating residential proxy
# or
npx mlx-labs search rotating residential proxy
```
...
# Plug rotating residential proxy provider — implement the flow with the endpoints printed above
```
...
// plug rotating residential proxy provider — wire to your scheduler / queue
```
...
**Keywords:** multilogin api cookbook · rotating residential proxy
